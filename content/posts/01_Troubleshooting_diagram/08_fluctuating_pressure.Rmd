...
ss.ceDiag(effect, causes.gr, causes, main = "Fluctuating Pressure", sub = "",
          ss.col = c("#8ad472", "#8ad472", "#8ad472", "#8ad472","#8ad472"))
```


```{r SixSigma package, echo= F }
effect <- "Fluctuating Pressure"
causes.gr <- c("Check Valves", "Pump Head")
causes <- vector(mode = "list", length = length(causes.gr))
causes[1] <- list(c("---Malfunctioning check valves"))
causes[2] <- list(c("---Gas bubbles"))
ss.ceDiag(effect, causes.gr, causes, main = "Fluctuating Pressure", sub = "",
          ss.col = c("#8ad472", "#8ad472", "#8ad472", "#8ad472","#8ad472"))
```
