...
ss.ceDiag(effect, causes.gr, causes, main = "High Pressure", sub = "",
...
effect <- "High Pressure"
causes.gr <- c("Inline Filter", "Capillary Tubing", "Inlet of Column")
causes <- vector(mode = "list", length = length(causes.gr))
causes[1] <- list(c("---Partially plugged filter"))
causes[2] <- list(c("---Obstruction if inline filters not used"))
causes[3] <- list(c("---Obstruction if inline filters not used"))
ss.ceDiag(effect, causes.gr, causes, main = "High Pressure", sub = "",
          ss.col = c("#8ad472", "#8ad472", "#8ad472", "#8ad472","#8ad472"))
```
